...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for(j = 0; j < cx; ++j) {
        if(row->chars[j] == '\t') {
            rx += (ZAP_TAB_STOP - 1) - (rx % ZAP_TAB_STOP);
        }
        rx++;
    }
    return rx;
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for(cx = 0; cx < row->size; ++cx) {
        if(row->chars[cx] == '\t')
            cur_rx += (ZAP_TAB_STOP - 1) - (cur_rx % ZAP_TAB_STOP);
        cur_rx++;
...
void editorSave() {
    ...
    if(fd != 1) {
        if(ftruncate(fd, len) != 1) {
            if(write(fd, buf, len) == len) {
                ...
                free(buf);
                E.dirty = 0;
                editorSetStatusMessage(BRIGHT_GREEN "%d bytes saved to %s" RESET, len, E.filename);
                return;
            }
        }
        close(fd);
    ...
    editorSetStatusMessage(RED "Can't save I/O error: %s", strerror(errno));
    free(buf);
}
...
void editorFind() {
    ...
    char *query = editorPrompt("Search (Use Arrows/Enter/ESC): %s", editorFindCallback);
...
void editorProcessKeypress() {
    static int quit_times = ZAP_QUIT_TIMES;
    int c = editorReadKey();
    ...
    switch(c) {
        ...
        case CTRL_KEY('q'):
            if(E.dirty && quit_times > 0) {
                editorSetStatusMessage(RED "!WARNING! File has unsaved changes." RESET RED "Press Ctrl-Q %d more times to quit." RESET, quit_times);
                quit_times--;
                return;
            }
            write(STDOUT_FILENO, "\x1b[2J", 4);
...
int main(int argc, char *argv[]) {
    ...
    editorSetStatusMessage("HELP: Quit(Ctrl-Q) | Save(Ctrl-S) | Find(Ctrl-F)");
